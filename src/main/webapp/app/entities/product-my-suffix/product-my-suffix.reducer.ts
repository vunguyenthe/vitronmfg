...
import { IProductMySuffix, defaultValue } from 'app/shared/model/product-my-suffix.model';
...
const initialState = {
  loading: false,
  errorMessage: null,
  entities: [] as ReadonlyArray<IProductMySuffix>,
  entity: defaultValue,
  updating: false,
  totalItems: 0,
  updateSuccess: false
...
export type ProductMySuffixState = Readonly<typeof initialState>;

// Reducer

export default (state: ProductMySuffixState = initialState, action): ProductMySuffixState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<IProductMySuffix> = (page, size, sort) => {
  const requestUrl = `${apiUrl}${sort ? `?page=${page}&size=${size}&sort=${sort}` : ''}`;
  return {
    type: ACTION_TYPES.FETCH_PRODUCT_LIST,
    payload: axios.get<IProductMySuffix>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};

export const getEntity: ICrudGetAction<IProductMySuffix> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_PRODUCT,
    payload: axios.get<IProductMySuffix>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<IProductMySuffix> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<IProductMySuffix> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<IProductMySuffix> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
